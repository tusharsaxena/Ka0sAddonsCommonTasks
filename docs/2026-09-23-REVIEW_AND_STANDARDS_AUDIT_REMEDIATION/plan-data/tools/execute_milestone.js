...
// Input: args.items = [{id, repo, deps, note?, branch?}] (note: an owner ruling handed to every agent;
//        branch: build on a candidate branch instead of BR), (hard deps: a failed dep skips the item), or
...
${it.branch ? `- WORK BRANCH: this item is built on the candidate branch ${it.branch}, not on ${BR}. If ${it.branch} does not exist, create it from ${BR}'s tip (\`git checkout -b ${it.branch} ${BR}\`, only from a clean tree on ${BR}); otherwise check it out. Commit there, leave ${it.branch} checked out, and never merge it into ${BR}: the owner's in-client capture decides that. Wherever this prompt says ${BR} for YOUR commits (landed check, commit, review), read ${it.branch}; dependency checks still read ${BR}.` : `- The repo must be on branch ${BR}; if it is not, stop and return blocked.`} Only touch ${it.repo} (other sibling repos are read-only).${it.note ? `\n- OWNER RULING FOR THIS ITEM (binding; it overrides any "blocked on" text in the item): ${it.note}` : ''} Never push, never merge, never amend or rebase existing commits, never edit an addon's libs/ or tests/_kit/ (the ONLY exception is an RV-* re-vendor item, which replaces them whole from the LibKa0s tag).
...
- Heavy runs MUST go through the bounded runner: \`${KB} luacheck .\`, \`${KB} lua5.1 tests/run.lua\`, \`${KB} lizard -l lua -x "./libs/*" -x "./tests/_kit/*" .\` (a hook blocks unbounded luacheck/lua/lizard). Gates: lint clean, tests green, no function above CCN 15, no authored .lua file above 1500 lines, plus the item's own verify commands. Exception for RV-* only: reds caused by the stricter new kit/library are allowed IF listed in the commit body.`
...
Check, against the item spec, the findings and 03_SPEC.md: does the change actually resolve every finding it claims, correctly? Any bug, regression, missed ripple (version/minor, docs/api, test-cases.md, changelog), test that does not really pin the behaviour, standards deviation introduced, or collateral edit? For an RV-* item: are libs/LibKa0s and tests/_kit byte-identical to the v1.56.0 tag, is the provenance line rolled, is the docs/revendor bundle present with the exact line 1, and are any reds listed in the commit body? Re-run the gates yourself. Do NOT edit or commit anything. If and only if ok=true, record the review so a resumed run can see it: \`git notes --ref=ka0s-review add -f -m "reviewed ok ${it.id}" <the item's LAST commit>\` (a note, not a commit). Report only REAL problems with evidence (file:line); stylistic preferences are not issues. ok=true if none.`, { label: `review:${it.id}`, phase: 'Review', schema: REVIEW })
